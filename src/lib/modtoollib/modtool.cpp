...
static Compat::Path get_python_root() {
    return Compat::Path(get_application_folder()) / "buildtools" / PYTHONDIR /
           "Python27";
}
...
static void initialize_compatibility_layer() {
     ...
     *
     * Works pretty much like the site.py script, but removes its dependency
     * under Unix. (and allows using a system wide Python installation instead
     * of a custom one)
     */
    ...
    Compat::Path site_custom_pythonpath =
        basic_custom_pythonpath / "site-packages";
    std::string new_pythonpath =
        basic_custom_pythonpath + ":" + site_custom_pythonpath;
...
char const *get_application_folder() {
    if (appplication_folder[0] == '\0') {
        error("ERROR: the application folder has not been set before calling "
    ...
    if (gTempFolder[0] == '\0') {
        ...
        if (env_path) {
            if (strlen(env_path) >= sizeof(gTempFolder)) {
                error("ERROR: buffer overflow on setting temp directory from "
                      "environment.");
            }
...
char *get_asset_temp_dir() {
    ...
    if (!created) {
        if (assetName[0] == '\0') {
            error("ERROR: the asset name has not been specified before "
                  "get_asset_temp_dir() was called.");
        }
...
void create_temp_dir() { (void)get_temp_dir(); }
    ...
    if (gLogPath[0] == '\0') {
        ...
        if (env_path) {
            if (strlen(env_path) >= sizeof(gLogPath)) {
                error("ERROR: buffer overflow on setting log path from "
                      "environment.");
            }
...
static void vlog_and_fprint(FILE *print_stream, char const *format,
                            va_list ap) {
    va_list ap1;
    va_copy(ap1, ap);
...
char const *get_python() {
    if (cached_python_path.empty()) {
        ...
        for (size_t i = 0; i < sizeof(possibilities) / sizeof(possibilities[0]);
             i++) {
            std::string attempt =
                std::string() + "which " + possibilities[i] + " &>/dev/null";
