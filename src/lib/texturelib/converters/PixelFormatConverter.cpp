...
char *DXTCompress(PixelFormat::Type pixel_format, const InputImage &image,
    ...
    // u8 const* rgba
    squish::CompressImage(reinterpret_cast<squish::u8 *>(image.RawData()), w, h,
                          compressed_data, flags);
...
char *DXTtoARGB(PixelFormat::Type pixel_format, uint32_t width, uint32_t height,
                const void *data) {
    uint32_t format = 0;
    ...
    squish::DecompressImage(reinterpret_cast<squish::u8 *>(decompressed), width,
                            height, data, format);

    // Squish decompresses to RGBA but we need ARGB eventually for display
    // purposes
...
char *ConvertToARGB8(const InputImage &image, uint32_t &data_size,
                     ePlatform platform) {
...
char *ConvertToRGB8(const InputImage &image, uint32_t &data_size,
                    ePlatform platform) {
...
char *ConvertToATITC(PixelFormat::Type pixel_format, const InputImage &image,
                     uint32_t &data_size) {
    TQonvertImage q_src;
...
char *ATITCtoARGB(PixelFormat::Type pixel_format, uint32_t width,
                  uint32_t height, const void *data, const uint32_t data_size) {
    TQonvertImage q_src;
    memset(&q_src, 0, sizeof(TQonvertImage));
    ...
    q_src.pData = const_cast<unsigned char *>(
...
char *ConvertToPVRTC(PixelFormat::Type pixel_format, const InputImage &image,
                     uint32_t &data_size) {
    ...
    CPVRTextureHeader header(PVRStandard8PixelType.PixelTypeID,
    ...
    bool ret = Transcode(tex, pvr_format, ePVRTVarTypeUnsignedByteNorm,
                         ePVRTCSpacelRGB);
    if (!ret) {
        throw;
    }
...
char *PVRTCtoARGB(PixelFormat::Type pixel_format, uint32_t width,
    ...
    PVRTDecompressPVRTC(data, pvr2_bit, width, height,
...
char *Convert(PixelFormat::Type pixel_format, const InputImage &image,
              uint32_t &data_size, ePlatform platform) {
    char *result = NULL;
...
uint32_t BytesPerBlock(PixelFormat::Type pixel_format) {
    ...
    case PixelFormat::BC1: {
        ...
        bytes_per_block =
            PIXELS_PER_DXT_BLOCK * num_bits_per_dxt1_block / BITS_PER_BYTE;
    } break;
    ...
    case PixelFormat::BC3: {
        ...
        bytes_per_block =
            PIXELS_PER_DXT_BLOCK * num_bits_per_dxt5_block / BITS_PER_BYTE;
    } break;
...
void MinTextureDimensions(PixelFormat::Type pixel_format, uint32_t &w,
                          uint32_t &h) {
    switch (pixel_format) {
    case PixelFormat::BC1:
    case PixelFormat::BC2:
    case PixelFormat::BC3:
        // OpenGL actually requires that you have a complete mip chain but you
        // must specify whatever stride/block size and pad
        w = 1;
        h = 1;
        break;
