...
TextureOpenGL::TextureOpenGL() : ToolTexture(0) {
    mMetaData.mPlatform = PLATFORM_OPENGL;
}

TextureOpenGL::TextureOpenGL(uint32_t num_mips) : ToolTexture(num_mips) {
    mMetaData.mPlatform = PLATFORM_OPENGL;
}

TextureOpenGL::~TextureOpenGL() {}

TextureOpenGL *TextureOpenGL::Convert(TextureType::Type type,
                                      PixelFormat::Type pixel_format,
    ...
    for (uint32_t i = 0; i < num_levels; ++i) {
        ...
        const void *converted_pixel_data = PixelFormatConverter::Convert(
            pixel_format, image, converted_size, PLATFORM_OPENGL);
        if (converted_pixel_data == NULL)
            throw;

        const uint32_t pitch = PixelFormatConverter::Pitch(pixel_format, image);

        texture->SetMipDescription(i, image.Width(), image.Height(), pitch,
                                   converted_size, converted_pixel_data);
    }
...
// You must ALWAYS return a copy of the data or a decompressed version of the
// data. The external system WILL be calling back into the library to free the
// buffer.
void *TextureOpenGL::ToARGB(uint32_t mip_level) const {
    ...
    case PixelFormat::BC3:
        result = PixelFormatConverter::DXTtoARGB(pixel_format, mip.mWidth,
                                                 mip.mHeight, mip.mData);
        break;
    ...
    case PixelFormat::A8R8G8B8: {
        ...
        for (uint32_t y = 0; y < mip.mHeight; ++y) {
            for (uint32_t x = 0; x < mip.mWidth;
                 ++x, dest_pixel += 4, src_pixel += 4) {
                char a = src_pixel[3];
                char b = src_pixel[2];
                char g = src_pixel[1];
                char r = src_pixel[0];
    ...
    case PixelFormat::ATITC_RGBA_INT:
        result = PixelFormatConverter::ATITCtoARGB(
            pixel_format, mip.mWidth, mip.mHeight, mip.mData, mip.mDataSize);
        break;
    ...
    case PixelFormat::PVRTC_RGBA_2:
        result = PixelFormatConverter::PVRTCtoARGB(pixel_format, mip.mWidth,
                                                   mip.mHeight, mip.mData);
        break;
    }

    // OpenGL data is stored bottom left corner pixel first rather than top left
    // corner pixel so we must flip the rows vertically.
