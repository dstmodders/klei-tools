...
InputImage::InputImage(const std::string &filename) : mDIB(NULL) {
...
InputImage::InputImage(FIBITMAP *dib) : mDIB(dib) {}

InputImage::InputImage(uint32_t w, uint32_t h, uint32_t bpp) {
    mDIB = FreeImage_Allocate(w, h, bpp);
}

InputImage::~InputImage() { FreeImage_Unload(mDIB); }

uint32_t InputImage::Width() const { return FreeImage_GetWidth(mDIB); }

uint32_t InputImage::Height() const { return FreeImage_GetHeight(mDIB); }

uint32_t InputImage::BPP() const { return FreeImage_GetBPP(mDIB); }

void *InputImage::RawData() const { return FreeImage_GetBits(mDIB); }
...
InputImage *InputImage::Resize(uint32_t w, uint32_t h,
...
void InputImage::FlipVertical() const { FreeImage_FlipVertical(mDIB); }
...
void InputImage::PremultiplyAlpha() {
    for (uint32_t y = 0; y < Height(); ++y) {
        for (uint32_t x = 0; x < Width(); ++x) {
            ...
            rgb_quad.rgbGreen =
                static_cast<BYTE>(alpha * rgb_quad.rgbGreen + 0.5);
            ;
            rgb_quad.rgbBlue =
                static_cast<BYTE>(alpha * rgb_quad.rgbBlue + 0.5);
            ;
...
bool InputImage::Paste(const InputImage *src, uint32_t top, uint32_t left,
                       uint32_t alpha) {
