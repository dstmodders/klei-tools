...
DLLEXPORT void *WINAPI LoadTexture(const char *filename) {
    ...
    if (fp != NULL) {
        ...
        if (bytes_read == file_size) {
            const ToolTexture *raw_texture =
                reinterpret_cast<const ToolTexture *>(file_data);
            ...
            case PLATFORM_XBOX360:
                ...
                // reader = new EndianSwappedBinaryBufferReader( file_size,
                // file_data );
                break;
...
DLLEXPORT void *WINAPI GetMip(void *raw_texture, int mip_level) {
    ...
    const ToolTexture::sMipDescription &mip_desc =
        texture->GetMipDescription(mip_level);
    return const_cast<ToolTexture::sMipDescription *>(&mip_desc);
}

DLLEXPORT int WINAPI GetMipWidth(void *mip) {
    const ToolTexture::sMipDescription *mip_desc =
        reinterpret_cast<const ToolTexture::sMipDescription *>(mip);
...
DLLEXPORT void WINAPI FreeRGBAData(void *data) { delete[] data; }
}
