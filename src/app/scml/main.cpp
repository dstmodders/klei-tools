...
typedef SCML::Data::Entity::Animation::Mainline::Key::Bone_Container
    s_bone_container;
typedef SCML_MAP(int, s_bone_container) s_bone_container_map;
typedef SCML::Data::Entity::Animation::Mainline::Key::Object_Container
    s_object_container;
...
template <typename T> struct PlanarPoint {
    T x;
    T y;
    ...
    template <typename U>
    explicit PlanarPoint(const PlanarPoint<U> &p)
...
struct bounding_box {
  ...
  public:
    ...
    explicit bounding_box(const rectangle &r) {
        set_values(r.x1, r.y1, int(ceilf(r.x2 - r.x1)),
                   int(ceilf(r.y2 - r.y1)));
    }
...
struct matrix3 {
    matrix3()
        : m00(1.f), m01(0.f), m02(0.f), m10(0.f), m11(1.f), m12(0.f), m20(0.f),
          m21(0.f), m22(1.f) {}
    ...
    float2 get_translation() const { return float2(m02, m12); }
...
float to_rads(float deg) { return deg * 2.0f * 3.14159265f / 360.f; }
...
void import_frame(IN s_file &file, OUT int &num, OUT int &duration,
                  OUT char *image_path, OUT int2 &dimensions, OUT float2 &pivot,
                  OUT int &id) {
    num = file.id;
    duration = 1;
...
void import_symbol(IN s_folder &folder, IN int symbol_index,
                   OUT char *symbol_name, OUT int &symbol_frame_count,
                   OUT int *frame_nums, OUT int *frame_durations,
                   OUT char **frame_image_paths, OUT int2 *frame_dimensions,
                   OUT float2 *frame_pivots, OUT symbol_id *ids) {
    ...
    int frame_index = 0;
    for (s_file_map::iterator file_iter = folder.files.begin();
         file_iter != folder.files.end(); ++file_iter) {
        symbol_id &id = ids[frame_index];
        id.folder = folder.id;

        s_file &file = *file_iter->second;
        import_frame(IN file, OUT frame_nums[frame_index],
                     OUT frame_durations[frame_index],
                     OUT frame_image_paths[frame_index],
                     OUT frame_dimensions[frame_index],
                     OUT frame_pivots[frame_index], OUT id.file);
        ++frame_index;
    }
}

void import_build(IN s_data &scml, OUT char *build_name,
                  OUT char **symbol_names, OUT int *symbol_frame_counts,
                  OUT int *symbol_frame_indices, OUT int *frame_nums,
                  OUT int *frame_durations, OUT char **frame_image_paths,
                  OUT int2 *frame_dimensions, OUT float2 *frame_pivots,
                  OUT symbol_id *ids) {
    ...
    for (s_folder_map::iterator iter = scml.folders.begin();
         iter != scml.folders.end(); ++iter) {
        s_folder &folder = *iter->second;
        symbol_frame_indices[symbol_index] = symbol_frame_index;
        import_symbol(IN folder, IN symbol_index,
...
void convert_image_path_to_name(IN char const *path, OUT char *name) {
...
void create_trans_rot_scale_pivot_matrix(IN float2 const &pos, IN float angle,
...
void convert_timeline_frame_to_element_frame(IN int2 const &image_dimensions,
                                             IN float2 const &pivot,
                                             IN float2 const &scale,
                                             IN float2 const &pos,
                                             IN float angle, OUT matrix3 &m) {
    ...
    create_trans_rot_scale_pivot_matrix(
        IN float2(pos.x, -pos.y), IN angle, IN scale,
        IN float2(-scaled_pivot_x, scaled_pivot_y), OUT m);
}

void convert_timeline_to_frames(
    IN int anim_length, IN int timeline_id,
    IN const std::set<std::pair<int, int> > &forceexport, IN bool looping,
    IN int frame_num, IN int key_count, IN int const *times,
    IN float2 const *positions, IN int2 const *dimensions,
    IN float2 const *pivots, IN float2 const *scales, IN float const *angles,
    IN int const *spins, IN float const *alphas, OUT float2 &frame_position,
    OUT int2 &frame_dimension, OUT float2 &frame_pivot, OUT float2 &frame_scale,
    OUT float &frame_angle, OUT float &frame_alpha, OUT int &start_key) {
    start_key = 0;

    bool should_export = true;

    if (key_count <= 0 || (!forceexport.count(make_pair(timeline_id, 0)) &&
                           times[0] > frame_num)) {
        should_export = false;
    } else if (!forceexport.count(make_pair(timeline_id, key_count - 1)) &&
               times[key_count - 1] + 1000 / FRAME_RATE < frame_num) {
    ...
    if (start_key == key_count - 1 && looping &&
        times[start_key] != frame_num) {
        end_key = 0;
    ...
    if (end_time != start_time) {
        blend =
            (float)(frame_num - start_time) / (float)(end_time - start_time);
    }
    ...
    frame_angle =
        lerp_angle(angles[start_key], angles[end_key], blend, spins[end_key]);
    frame_alpha = lerp(alphas[start_key], alphas[end_key], blend);
}

void convert_anim_timelines_to_frames(
    IN int length, IN bool looping,
    IN const std::set<std::pair<int, int> > &timeline_key_forceexport,
    IN int timeline_count, IN int *timeline_key_start_indices,
    IN int *timeline_key_counts, IN int *timeline_key_times,
    IN float2 *timeline_key_positions, IN int2 *timeline_key_dimensions,
    IN float2 *timeline_key_pivots, IN float2 *timeline_key_scales,
    IN float *timeline_key_angles, IN int *timeline_key_z_indices,
    IN int *timeline_key_spins, IN int *timeline_key_symbol_frame_nums,
    IN char **timeline_key_names, IN int *timeline_key_parent_ids,
    IN float *timeline_key_alphas, IN char **timeline_names,
    OUT int &anim_frame_count, OUT int *frame_element_count,
    OUT int *frame_element_start_indices, OUT int *frame_indices,
    OUT float2 *frame_positions, OUT int2 *frame_dimensions,
    OUT int *timeline_frame_symbol_frame_nums, OUT char **timeline_frame_names,
    OUT char **timeline_frame_layer_names, OUT float2 *timeline_frame_positions,
    OUT int2 *timeline_frame_dimensions, OUT float2 *timeline_frame_pivots,
    OUT float2 *timeline_frame_scales, OUT float *timeline_frame_angles,
    OUT int *timeline_frame_z_indices, OUT int *timeline_frame_parent_ids,
    OUT float *timeline_frame_alphas, OUT int &element_index) {
    int frame_index = 0;
    for (int frame_num = 0; frame_num < length;
        ...
        for (int i = 0; i < timeline_count; ++i) {
            ...
            convert_timeline_to_frames(
                length, i, timeline_key_forceexport, looping, frame_num,
                timeline_key_counts[i], &timeline_key_times[key_start_index],
                ...
                timeline_frame_alphas[element_index], key_offset);
            ...
            strcpy(timeline_frame_names[element_index],
                   timeline_key_names[key_index]);
            strcpy(timeline_frame_layer_names[element_index],
                   timeline_layer_name.c_str());
            timeline_frame_z_indices[element_index] =
                timeline_key_z_indices[key_index];
            timeline_frame_symbol_frame_nums[element_index] =
                timeline_key_symbol_frame_nums[key_index];
            timeline_frame_parent_ids[element_index] =
                timeline_key_parent_ids[key_index];

            frame_dimensions[frame_index] = timeline_key_dimensions[key_index];

#ifdef ANIMDEBUG
            cout << "Key index is: " << key_index << " = " << key_start_index
                 << " + " << key_offset << endl;
            cout << "Set frame to: "
                 << timeline_key_symbol_frame_nums[key_index] << endl;
#endif
...
void export_symbol_frame(OUT symbol_frame_metadata_t &frame_metadata,
                         IN xml_writer &writer, IN int num, IN int duration,
                         IN char const *image, IN int2 const &dimensions,
...
void export_symbol(OUT symbol_metadata_t &symbol_metadata,
                   IN xml_writer &writer, IN char const *name,
                   IN int frame_count, IN int const *frame_nums,
    ...
    writer.push("Symbol").attribute("name", skip_slash(name));
    ...
    for (int i = 0; i < frame_count; ++i) {
        ...
        export_symbol_frame(OUT frame_metadata, IN writer, IN frame_nums[i],
                            IN frame_durations[i], IN frame_images[i],
                            IN frame_dimensions[i], IN frame_positions[i]);
    }
    writer.pop();
}

void export_build(OUT build_metadata_t &build_metadata,
                  IN char const *build_xml_path, IN char const *build_name,
                  IN int symbol_count, IN char const *const *symbol_names,
                  IN int const *symbol_frame_counts,
                  IN int const *symbol_frame_indices, IN int const *frame_nums,
                  IN int const *frame_durations,
                  IN char const *const *frame_images,
                  IN int2 const *frame_dimensions,
                  IN float2 const *frame_positions) {
    ...
    writer.push("Build").attribute("name", build_name);
    ...
    for (int i = 0; i < symbol_count; ++i) {
        ...
        export_symbol(OUT symbol_metadata, IN writer, IN symbol_names[i],
                      IN symbol_frame_counts[i], IN & frame_nums[frame_index],
...
void get_build_counts(IN s_data &scml, OUT int &symbol_count,
...
void export_element(IN xml_writer &writer, IN const char *name,
                    IN const char *layer_name, IN int frame, IN int z_index,
                    IN const matrix3 &m) {
    writer.push("element")
        .attribute("name", name)
...
// Per element.
static bool extend_bounding_box(IN const symbol_metadata_t &symbol_metadata,
                                IN const char *symbol_name, IN bool first,
                                OUT rectangle &rect, IN int frame,
                                IN const matrix3 &m) {
    if (symbol_metadata.size() <= frame) {
        log_and_fprint(stderr,
                       "WARNING: frame %0d of animation symbol %s is being "
                       "used by the animation but not defined by the build.\n");
        return false;
    }
...
void export_animation_frame(IN const build_metadata_t &build_metadata,
                            IN xml_writer &writer, IN int index,
                            IN int2 &dimensions, IN float2 &position,
                            IN int element_count, IN char **element_names,
                            IN char **element_layer_names,
                            IN int *element_frames, IN int *element_z_indices,
    ...
    for (int i = 0; i < element_count; ++i) {
        if (element_alphas[i] >= 0.5) {
            build_metadata_t::const_iterator symbol_searcher =
                build_metadata.find(element_names[i]);
            if (symbol_searcher != build_metadata.end()) {
                if (extend_bounding_box(
                        IN symbol_searcher->second, IN element_names[i],
                        IN uninitialized_rect, IN bounding_rectangle,
                        IN element_frames[i], IN element_matrices[i])) {
    ...
    for (int i = 0; i < element_count; ++i) {
        if (element_alphas[i] >= 0.5) {
            export_element(IN writer, IN element_names[i],
                           IN element_layer_names[i], IN element_frames[i],
                           IN element_z_indices[i], IN element_matrices[i]);
        }
    }
...
void export_animation(IN const build_metadata_t &build_metadata,
                      IN xml_writer &writer, IN char *name, IN char *root,
                      IN int num_frames, IN int framerate,
                      IN int *frame_indices, IN int2 *frame_dimensions,
                      IN float2 *frame_positions, IN int *frame_element_counts,
                      IN int *frame_element_start_indices,
                      IN char **element_names, IN char **element_layer_names,
                      IN int *element_frames, IN int *element_z_indices,
                      IN float *element_alphas, IN matrix3 *element_matrices) {
    writer.push("anim")
        .attribute("name", name)
        .attribute("root", root)
    ...
    for (int i = 0; i < num_frames; ++i) {
        ...
        export_animation_frame(
            IN build_metadata, IN writer, IN frame_indices[i],
            IN frame_dimensions[i], IN frame_positions[i],
            IN frame_element_counts[i], IN & element_names[element_start_index],
...
bool is_bone_timeline(IN s_timeline &timeline) {
    return strcmp(timeline.object_type.c_str(), "bone") == 0;
}

void import_animation(IN s_animation &anim, OUT char *name, OUT int &num_frames,
                      OUT int &framerate, OUT bool &looping) {
...
void import_mainline_key(const s_object_ref &object_ref,
                         const s_animation &anim, const s_data &scml,
                         OUT float2 &position, OUT float2 &pivot,
                         OUT float &angle, OUT float2 &scale, OUT int &z_index,
                         OUT int &spin, OUT int &id, OUT int &timeline_id,
                         OUT int &parent_id) {
    id = object_ref.key;
    ...
    */
    ...
    s_timeline_map::const_iterator timeline_match =
        anim.timelines.find(timeline_id);
    if (timeline_match == anim.timelines.end() ||
        timeline_match->second == NULL) {
        return;
    }
    ...
    s_folder_map::const_iterator folder_match =
        scml.folders.find(object.folder);
...
bool is_valid_timeline(IN s_timeline &timeline) {
    return strcmp(timeline.object_type.c_str(), "bone") != 0;
}

void import_mainline(
    IN s_data &scml, OUT int *mainline_key_start_indices,
    OUT int *mainline_key_counts, OUT float2 *mainline_key_positions,
    OUT float2 *mainline_key_pivots, OUT float *mainline_key_angles,
    OUT float2 *mainline_key_scales, OUT int *mainline_key_z_indices,
    OUT int *mainline_key_spins, OUT int *mainline_key_ids,
    OUT int *mainline_key_timeline_ids, OUT int *parent_ids,
    OUT std::set<std::pair<int, int> > *anim_timeline_key_forceexport) {
    int anim_index = 0;
    int mainline_key_index = 0;
    for (s_entity_map::iterator iter = scml.entities.begin();
         iter != scml.entities.end(); ++iter) {
        s_entity &entity = *iter->second;
        for (s_animation_map::iterator anim_iter = entity.animations.begin();
             anim_iter != entity.animations.end(); ++anim_iter) {
            ...
            if (is_valid_animation(anim)) {
                ...
                std::set<std::pair<int, int> > &timeline_key_forceexport =
                    anim_timeline_key_forceexport[anim_index];
                ...
                for (s_mainline_key_map::iterator iter = keys.begin();
                     iter != keys.end(); ++iter) {
                    ...
                    for (s_object_container_map::iterator object_iter =
                             key.objects.begin();
                         object_iter != key.objects.end(); ++object_iter) {
                        s_object_ref &object = *object_iter->second.object_ref;
                        import_mainline_key(
                            IN object, IN anim, IN scml,
                ...
                mainline_key_counts[anim_index] =
                    mainline_key_index_end - mainline_key_index_begin;

                if (max_key != NULL) {
                    for (s_object_container_map::iterator object_iter =
                             max_key->objects.begin();
                         object_iter != max_key->objects.end(); ++object_iter) {
                        s_object_ref &object = *object_iter->second.object_ref;

                        timeline_key_forceexport.insert(
                            make_pair(object.timeline, object.key));
                    }
                }
...
void find_mainline_key_index(IN int &base_index, IN int &timeline_id,
                             IN int &key_id, IN int &key_count, IN int *key_ids,
                             IN int *timeline_ids, OUT int &key_index) {
...
void find_symbol_index(IN symbol_id &id, IN int count, IN symbol_id *ids,
                       OUT int &index) {
    index = 0;
...
void import_timeline_maps(IN s_data &scml, IN int *mainline_key_start_indices,
                          IN int *mainline_key_counts, IN int *mainline_key_ids,
                          IN int *mainline_key_timeline_ids,
                          IN int symbol_frame_count, IN symbol_id *symbol_ids,
...
#endif
            if (is_valid_animation(anim)) {
                for (s_timeline_map::iterator timeline_iter =
                         anim.timelines.begin();
                     timeline_iter != anim.timelines.end(); ++timeline_iter) {
#ifdef ANIMDEBUG
                    ...
                    if (is_valid_timeline(timeline)) {
                        ...
                        int mainline_start_index =
                            mainline_key_start_indices[anim_index];
                        find_mainline_key_index(
                            IN mainline_start_index, IN timeline.id,
                            IN timeline.keys.begin()->second->id,
                            IN mainline_key_counts[anim_index],
                            IN & mainline_key_ids[mainline_start_index],
                            IN &
                                mainline_key_timeline_ids[mainline_start_index],
                            OUT first_mainline_key_index);

                        int first_key_id = timeline.keys.begin()->second->id;
                        for (s_timeline_key_map::iterator key_iter =
                                 timeline.keys.begin();
                             key_iter != timeline.keys.end(); ++key_iter) {
#ifdef ANIMDEBUG
                            ...
                            find_mainline_key_index(
                                IN mainline_start_index, IN timeline.id,
                                IN key.id, IN mainline_key_counts[anim_index],
                                IN & mainline_key_ids[mainline_start_index],
                            ...
                            timeline_mainline_first_key_map
                                [timeline_key_index] = first_mainline_key_index;

                            symbol_id sym_id;
                            sym_id.folder = object.folder;
                            sym_id.file = object.file;
                            find_symbol_index(
                                sym_id, symbol_frame_count, symbol_ids,
                                timeline_symbol_map[timeline_key_index]);

#ifdef ANIMDEBUG
                            const size_t sym_index =
                                timeline_symbol_map[timeline_key_index];
                            cout << "Got symbol index "
                                 << timeline_symbol_map[timeline_key_index]
                                 << " (folder: " << symbol_ids[sym_index].folder
                                 << ", file: " << symbol_ids[sym_index].file
                                 << ")" << endl;
#endif
...
void import_timeline_key(IN s_timeline_key &key, OUT float2 &position,
                         OUT float2 &pivot, OUT float &angle, OUT float2 &scale,
                         OUT int &spin, OUT int &time,
                         OUT int &symbol_frame_num, OUT float &alpha) {
    s_timeline_object &object = key.object;
...
void import_timelines(
    IN s_data &scml, OUT int *anim_timeline_counts,
    OUT int *anim_timeline_start_indices,
    OUT int *anim_timeline_key_start_indices, OUT int *anim_timeline_key_counts,
    OUT float2 *timeline_key_positions, OUT float2 *timeline_key_pivots,
    OUT float *timeline_key_angles, OUT float2 *timeline_key_scales,
    OUT int *timeline_key_spins, OUT int *timeline_key_times,
    OUT int *timeline_key_symbol_frame_nums, OUT char **timeline_key_names,
    OUT float *timeline_key_alphas, OUT char **timeline_names) {
    int anim_index = 0;
    int timeline_key_index = 0;
        ...
        for (s_animation_map::iterator anim_iter = entity.animations.begin();
            ...
            if (is_valid_animation(anim)) {
                ...
                for (s_timeline_map::iterator timeline_iter =
                    ...
                    s_timeline &timeline = *timeline_iter->second;
                    if (is_valid_timeline(timeline)) {
                        anim_timeline_key_start_indices[timeline_index] =
                            timeline_key_index;
                        anim_timeline_key_counts[timeline_index] =
                            timeline.keys.size();
                        for (s_timeline_key_map::iterator key_iter =
                                 timeline.keys.begin();
                             key_iter != timeline.keys.end(); ++key_iter) {
                            ...
                            s_folder &folder =
                                *scml.folders.find(key.object.folder)->second;
                            if (timeline.name.size() > 0) {
                                strcpy(timeline_names[timeline_key_index],
                                       timeline.name.c_str());
                            }

                            if (folder.name.size() > 0) {
                                strcpy(timeline_key_names[timeline_key_index],
                                       skip_slash(folder.name.c_str()));
                            } else {
                                sprintf(timeline_key_names[timeline_key_index],
                                        "symbol%i", folder.id);
                            }
...
void flatten(IN bone &b, IN int bone_count, OUT bone *bones) {
    if (b.is_flattened || b.parent_id == -1) {
        return;
    }

    for (int parent_bone_index = 0; parent_bone_index < bone_count;
         ++parent_bone_index) {
...
void import_bones(IN s_data &scml, OUT int *anim_bone_counts,
                  OUT int *anim_bone_start_indices,
                  OUT int *anim_bone_key_start_indices,
                  OUT int *anim_bone_key_counts, OUT bone *bones) {
    int anim_index = 0;
    int timeline_key_index = 0;
    int timeline_index = 0;
    for (s_entity_map::iterator iter = scml.entities.begin();
            ...
            if (is_valid_animation(anim)) {
                ...
                for (s_timeline_map::iterator timeline_iter =
                    ...
                    if (is_bone_timeline(timeline)) {
                        anim_bone_key_start_indices[timeline_index] =
                            timeline_key_index;
                        anim_bone_key_counts[timeline_index] =
                            timeline.keys.size();
                        for (s_timeline_key_map::iterator key_iter =
                                 timeline.keys.begin();
                             key_iter != timeline.keys.end(); ++key_iter) {
                            ...
                            for (s_bone_container_map::iterator
                                     mainline_bone_iter =
                                         anim.mainline.keys.begin()
                                             ->second->bones.begin();
                                 mainline_bone_iter !=
                                 anim.mainline.keys.begin()
                                     ->second->bones.end();
                                 ++mainline_bone_iter) {
                                if (mainline_bone_iter->second.bone_ref
                                        ->timeline == timeline.id) {
                                    b.id =
                                        mainline_bone_iter->second.bone_ref->id;
                                    b.parent_id = mainline_bone_iter->second
                                                      .bone_ref->parent;
                                    found_bone = true;
                                    break;
                                }
                            ...
                            if (!found_bone) {
                                error("ERROR: Bone '%s' doesn't exist on first "
                                      "key frame.",
                                      timeline.name.c_str());
                            }
...
void import_animations(IN s_data &scml, OUT char **names, OUT char **roots,
                       OUT int *num_frames, OUT int *framerates,
                       OUT bool *loopings) {
    int anim_index = 0;
    for (s_entity_map::iterator iter = scml.entities.begin();
         iter != scml.entities.end(); ++iter) {
        ...
        for (s_animation_map::iterator anim_iter = entity.animations.begin();
            ...
            if (is_valid_animation(anim)) {
                ...
                import_animation(anim, names[anim_index],
                                 num_frames[anim_index], framerates[anim_index],
                                 loopings[anim_index]);
                ++anim_index;
            }
        }
...
void get_anim_counts(IN s_data &scml, OUT int &anim_count, OUT int &frame_count,
                     OUT int &element_count, OUT int &timeline_count,
                     OUT int &mainline_key_count, OUT int &timeline_key_count,
                     OUT int &bone_count, OUT int &bone_key_count,
    ...
    for (s_entity_map::iterator iter = scml.entities.begin();
        ...
        for (s_animation_map::iterator anim_iter = entity.animations.begin();
            ...
            if (is_valid_animation(anim)) {
                ...
                for (s_mainline_key_map::iterator iter =
                         anim.mainline.keys.begin();
                     iter != anim.mainline.keys.end(); ++iter) {
                    s_mainline_key &key = *iter->second;
                    mainline_key_count += key.objects.size();
                }

                for (s_timeline_map::iterator iter = anim.timelines.begin();
                     iter != anim.timelines.end(); ++iter) {
...
void apply_mainline_keys(IN int timeline_key_count,
    ...
    for (int i = 0; i < timeline_key_count; ++i) {
        timeline_key_z_indices[i] =
            mainline_key_z_indices[timeline_mainline_key_map[i]];
    }
}

void apply_mainline_keys(
    IN int timeline_key_count, IN int *timeline_mainline_key_map,
    IN float2 *mainline_key_positions, IN float2 *mainline_key_pivots,
    IN float *mainline_key_angles, IN float2 *mainline_key_scales,
    IN int *mainline_key_z_indices, IN int *mainline_key_spins,
    OUT float2 *timeline_key_positions, OUT float2 *timeline_key_pivots,
    OUT float *timeline_key_angles, OUT float2 *timeline_key_scales,
    OUT int *timeline_key_z_indices, OUT int *timeline_key_spins) {
    for (int i = 0; i < timeline_key_count; ++i) {
        timeline_key_positions[i] =
            mainline_key_positions[timeline_mainline_key_map[i]];
        timeline_key_pivots[i] =
            mainline_key_pivots[timeline_mainline_key_map[i]];
        timeline_key_angles[i] =
            mainline_key_angles[timeline_mainline_key_map[i]];
        timeline_key_scales[i] =
            mainline_key_scales[timeline_mainline_key_map[i]];
        timeline_key_z_indices[i] =
            mainline_key_z_indices[timeline_mainline_key_map[i]];
        timeline_key_spins[i] =
            mainline_key_spins[timeline_mainline_key_map[i]];
    }
}

void apply_mainline_parent_ids(IN int timeline_key_count,
                               IN int *timeline_mainline_key_map,
                               IN int *mainline_parent_ids,
                               IN int *timeline_key_parent_ids) {
    for (int i = 0; i < timeline_key_count; ++i) {
        timeline_key_parent_ids[i] =
            mainline_parent_ids[timeline_mainline_key_map[i]];
    }
}

void apply_symbols(IN float2 *symbol_frame_pivots,
                   IN int2 *symbol_frame_dimensions,
                   IN int *timeline_symbol_map, IN int timeline_key_count,
    ...
    for (int i = 0; i < timeline_key_count; ++i) {
        ...
        timeline_key_dimensions[i] =
            symbol_frame_dimensions[timeline_symbol_map[i]];
    }
}
...
void convert_z_index(IN int max_z, OUT int &z) { z = max_z - z; }

void convert_symbol_frame_pivots_to_build_pivots(
    IN int symbol_frame_count, IN int2 *symbol_frame_dimensions,
    IN float2 *symbol_frame_pivots, OUT float2 *build_frame_pivots) {
    for (int i = 0; i < symbol_frame_count; ++i) {
        float scaled_pivot_x =
            symbol_frame_pivots[i].x * float(symbol_frame_dimensions[i].x);
        float scaled_pivot_y =
            symbol_frame_pivots[i].y * float(symbol_frame_dimensions[i].y);

        build_frame_pivots[i].x =
            (float)symbol_frame_dimensions[i].x / 2.f - scaled_pivot_x;
        build_frame_pivots[i].y =
            (float)symbol_frame_dimensions[i].y / 2.f - scaled_pivot_y;
        build_frame_pivots[i].y = -build_frame_pivots[i].y;
    }
}

void apply_inverse_symbol_frame_pivots_to_timeline_pivots(
    IN float2 *symbol_frame_pivots, IN int timeline_key_count,
    IN int *timeline_symbol_map, IN float2 *timeline_key_pivots,
    OUT float2 *adjusted_timeline_key_pivots) {
    for (int i = 0; i < timeline_key_count; ++i) {
        adjusted_timeline_key_pivots[i].x =
            timeline_key_pivots[i].x -
            symbol_frame_pivots[timeline_symbol_map[i]].x;
        adjusted_timeline_key_pivots[i].y =
            timeline_key_pivots[i].y -
            symbol_frame_pivots[timeline_symbol_map[i]].y;
    }
}

void build_scml(IN s_data &scml, OUT char **&image_paths,
                OUT int &image_path_count) {
    // import build
    int symbol_count = 0;
    int symbol_frame_count = 0;

    get_build_counts(IN scml, OUT symbol_count, OUT symbol_frame_count);
    ...
    char **symbol_frame_image_paths =
        allocate_strings(symbol_frame_count, MAX_NAME_LENGTH);
    char **symbol_frame_image_names =
        allocate_strings(symbol_frame_count, MAX_NAME_LENGTH);
    ...
    import_build(IN scml, OUT build_name, OUT symbol_names,
                 OUT symbol_frame_counts, OUT symbol_frame_indices,
                 OUT symbol_frame_nums, OUT symbol_frame_durations,
                 OUT symbol_frame_image_paths, OUT symbol_frame_dimensions,
                 OUT symbol_frame_pivots, OUT symbol_ids);
    ...
    get_anim_counts(IN scml, OUT anim_count, OUT frame_count, OUT element_count,
                    OUT timeline_count, OUT mainline_key_count,
                    OUT timeline_key_count, OUT bone_count, OUT bone_key_count,
    ...
    char **timeline_key_names =
        allocate_strings(timeline_key_count, MAX_NAME_LENGTH);
    char **timeline_names =
        allocate_strings(timeline_key_count, MAX_NAME_LENGTH);
    ...
    std::set<std::pair<int, int> > *anim_timeline_key_forceexport =
        new std::set<std::pair<int, int> >[anim_count];

    import_animations(IN scml, OUT anim_names, OUT anim_roots, OUT anim_lengths,
                      OUT anim_framerates, OUT anim_loopings);

    import_mainline(IN scml, OUT mainline_key_start_indices,
                    OUT mainline_key_counts, OUT mainline_key_positions,
                    OUT mainline_key_pivots, OUT mainline_key_angles,
                    OUT mainline_key_scales, OUT mainline_key_z_indices,
                    OUT mainline_key_spins, OUT mainline_key_ids,
                    OUT mainline_key_timeline_ids, OUT mainline_parent_ids,
                    OUT anim_timeline_key_forceexport);

    import_timeline_maps(IN scml, IN mainline_key_start_indices,
                         IN mainline_key_counts, IN mainline_key_ids,
                         IN mainline_key_timeline_ids, IN symbol_frame_count,
                         IN symbol_ids, OUT timeline_mainline_first_key_map,
                         OUT timeline_mainline_key_map,
                         OUT timeline_symbol_map);

    apply_symbols(IN symbol_frame_pivots, IN symbol_frame_dimensions,
                  IN timeline_symbol_map, IN timeline_key_count,
                  OUT timeline_key_pivots, OUT timeline_key_dimensions);

    apply_mainline_keys(IN timeline_key_count,
                        IN timeline_mainline_first_key_map,
                        IN mainline_key_positions, IN mainline_key_pivots,
                        IN mainline_key_angles, IN mainline_key_scales,
                        IN mainline_key_z_indices, IN mainline_key_spins,
                        OUT timeline_key_positions, OUT timeline_key_pivots,
                        OUT timeline_key_angles, OUT timeline_key_scales,
                        OUT timeline_key_z_indices, OUT timeline_key_spins);

    apply_mainline_keys(IN timeline_key_count, IN timeline_mainline_key_map,
                        IN mainline_key_z_indices, OUT timeline_key_z_indices);

    apply_mainline_parent_ids(timeline_key_count, timeline_mainline_key_map,
                              mainline_parent_ids, timeline_key_parent_ids);

    import_timelines(
        IN scml, OUT anim_timeline_counts, OUT anim_timeline_start_indices,
        OUT anim_timeline_key_start_indices, OUT anim_timeline_key_counts,
        OUT timeline_key_positions, OUT timeline_key_pivots,
        OUT timeline_key_angles, OUT timeline_key_scales,
        OUT timeline_key_spins, OUT timeline_key_times,
        OUT timeline_key_symbol_frame_nums, OUT timeline_key_names,
        OUT timeline_key_alphas, OUT timeline_names);

    import_bones(IN scml, OUT anim_bone_counts, OUT anim_bone_start_indices,
                 OUT anim_bone_key_start_indices, OUT anim_bone_key_counts,
                 OUT anim_bone_keys);

    // conversion
    for (int i = 0; i < symbol_frame_count; ++i) {
        convert_image_path_to_name(IN symbol_frame_image_paths[i],
                                   OUT symbol_frame_image_names[i]);
    }

    for (int i = 0; i < timeline_key_count; ++i) {
        int max_z = timeline_count;
        convert_z_index(IN max_z, OUT timeline_key_z_indices[i]);
    }

    apply_inverse_symbol_frame_pivots_to_timeline_pivots(
        IN symbol_frame_pivots, IN timeline_key_count, IN timeline_symbol_map,
        IN timeline_key_pivots, OUT timeline_key_pivots);

    convert_symbol_frame_pivots_to_build_pivots(
        IN symbol_frame_count, IN symbol_frame_dimensions,
        IN symbol_frame_pivots, OUT build_frame_pivots);
    ...
    char **timeline_frame_names =
        allocate_strings(element_count, MAX_NAME_LENGTH);
    char **timeline_frame_layer_names =
        allocate_strings(element_count, MAX_NAME_LENGTH);
    ...
    for (int i = 0; i < anim_count; ++i) {
        ...
        convert_anim_timelines_to_frames(
            anim_lengths[i], anim_loopings[i], anim_timeline_key_forceexport[i],
            anim_timeline_counts[i],
            &anim_timeline_key_start_indices[timeline_start_index],
            &anim_timeline_key_counts[timeline_start_index], timeline_key_times,
            timeline_key_positions, timeline_key_dimensions,
            timeline_key_pivots, timeline_key_scales, timeline_key_angles,
            timeline_key_z_indices, timeline_key_spins,
            timeline_key_symbol_frame_nums, timeline_key_names,
            timeline_key_parent_ids, timeline_key_alphas, timeline_names,
            anim_frame_counts[i],
            ...
            timeline_frame_symbol_frame_nums, timeline_frame_names,
            timeline_frame_layer_names, timeline_frame_positions,
            timeline_frame_dimensions, timeline_frame_pivots,
            timeline_frame_scales, timeline_frame_angles,
            timeline_frame_z_indices, timeline_frame_parent_ids,
    ...
    for (int anim_index = 0; anim_index < anim_count; ++anim_index) {
        ...
        int *bone_key_start_indices =
            &anim_bone_key_start_indices[anim_bone_start_indices[anim_index]];
        int *bone_key_counts =
            &anim_bone_key_counts[anim_bone_start_indices[anim_index]];
        anim_bone_frame_start_indices[anim_index] = bone_frame_index;
        for (int frame_num = 0; frame_num < anim_length;
             frame_num += 1000 / FRAME_RATE) {
            for (int bone_index = 0; bone_index < bone_count; ++bone_index) {
                bone *bone_keys =
                    &anim_bone_keys[bone_key_start_indices[bone_index]];
                int bone_key_count = bone_key_counts[bone_index];

                int start_key = 0;
                int end_key = 0;
                for (int bone_key_index = 0; bone_key_index < bone_key_count;
                     ++bone_key_index) {
                ...
                if (start_key == bone_key_count - 1 && looping &&
                ...
                float blend = 0.f;
                if (end_time != start_time) {
                    blend = (float)(frame_num - start_time) /
                ...
                create_trans_rot_scale_pivot_matrix(
                    IN float2(bone_frame.x, bone_frame.y), IN bone_frame.angle,
                    IN float2(bone_frame.scale_x, bone_frame.scale_y),
                    IN float2(0, 0), OUT bone_frame.flattened);
            }
        }
    }
    ...
    for (int anim_index = 0; anim_index < anim_count; ++anim_index) {
        ...
        for (int frame_num = 0; frame_num < anim_length;
             frame_num += 1000 / FRAME_RATE) {
            bone *bone_frames =
                &anim_bone_frames[anim_bone_frame_start_indices[anim_index] +
                                  frame_index * bone_count];
            bone *flattened_bone_frames =
                &anim_flattened_bone_frames
                    [anim_bone_frame_start_indices[anim_index] +
                     frame_index * bone_count];
            for (int bone_index = 0; bone_index < bone_count; ++bone_index) {
            ...
            for (int bone_index = 0; bone_index < bone_count; ++bone_index) {
                flatten(flattened_bone_frames[bone_index], bone_count,
    ...
    for (int anim_index = 0; anim_index < anim_count; ++anim_index) {
        ...
        if (anim_length > 0) {
            ...
            int *anim_frame_element_counts =
                &frame_element_counts[frame_start_index];
            int *anim_frame_element_start_indices =
                &frame_element_start_indices[frame_start_index];

            int bone_count = anim_bone_counts[anim_index];
            for (int frame_index = 0;
                 frame_index < anim_length / (1000 / FRAME_RATE);
                 ++frame_index) {
                bone *bones = &anim_flattened_bone_frames
                                  [anim_bone_frame_start_indices[anim_index] +
                                   frame_index * bone_count];
                int element_start_index =
                    anim_frame_element_start_indices[frame_index];
                ...
                float2 *positions =
                    &timeline_frame_positions[element_start_index];
                ...
                for (int element_index = 0; element_index < element_count;
                    ...
                    if (parents[element_index] >= 0) {
                        for (int bone_index = 0; bone_index < bone_count;
                             ++bone_index) {
    ...
    for (int i = 0; i < element_count; ++i) {
        convert_timeline_frame_to_element_frame(
            IN timeline_frame_dimensions[i], IN timeline_frame_pivots[i],
            IN timeline_frame_scales[i], IN timeline_frame_positions[i],
            IN timeline_frame_angles[i], OUT element_matrices[i]);
    }
    ...
    if (build_anim_index >= 0) {
        bool character_build =
            strcmp(anim_names[build_anim_index], "BUILD_PLAYER") == 0;
    ...
    // export builds
    export_build(OUT build_metadata,
                 IN(Path(get_asset_temp_dir()) / "build.xml").c_str(),
                 IN build_name, IN symbol_count, IN symbol_names,
                 IN symbol_frame_counts, IN symbol_frame_indices,
                 IN symbol_frame_nums, IN symbol_frame_durations,
                 IN symbol_frame_image_names, IN symbol_frame_dimensions,
                 IN build_frame_pivots);

    // export animations
    if (anim_count > 0) {
        xml_writer animation_writer;
        animation_writer.begin_doc(
            (Path(get_asset_temp_dir()) / "animation.xml").c_str());
        ...
        for (int i = 0; i < anim_count; ++i) {
            if (anim_lengths[i] >= (1000 / FRAME_RATE)) {
                ...
                export_animation(
                    IN build_metadata, IN animation_writer, IN anim_names[i],
                    IN anim_roots[i], IN anim_lengths[i] / (1000 / FRAME_RATE),
                    ...
                    IN timeline_frame_names, IN timeline_frame_layer_names,
                    IN timeline_frame_symbol_frame_nums,
                    IN timeline_frame_z_indices, IN timeline_frame_alphas,
                    IN element_matrices);
            }
        }
...
int main(int argument_count, char **arguments) {
    ...
    bool ignore_self_date =
        extract_arg(argument_count, arguments, "ignore-self-date");
    ...
    Path built_package_path =
        output_dir / "anim" / output_package_file_path.basename();
     ...
     * If neither compared files of a pair exist, the check fails, since the
     * inequality is strict.
     */
    if (!force && built_package_path.isNewerThan(input_file_path) &&
    ...
    sprintf(command_line,
            ...
            get_asset_temp_dir(), output_package_file_path.c_str());
    run(command_line, true, "Packaging '%s'",
        output_package_file_path.basename().c_str());

    sprintf(command_line,
#if defined(IS_WINDOWS)
            "\"\"%s\" \"%s\" --skip_update_prefabs --outputdir \"%s\" "
            "--prefabsdir \"%s\" \"%s\"\"",
#else
            "\"%s\" \"%s\" --skip_update_prefabs --outputdir \"%s\" "
            "--prefabsdir \"%s\" \"%s\"",
#endif
            get_python(), (app_folder / "exported" / "export.py").c_str(),
            output_dir.c_str(), (app_folder / "data").c_str(),
            output_package_file_path.c_str());
    run(command_line, true, "Building '%s'",
        output_package_file_path.basename().c_str());
