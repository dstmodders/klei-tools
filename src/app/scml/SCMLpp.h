...
// Be careful with these...  Macros don't nest well.  Use a typedef as necessary
// for the map parameters.
#define SCML_BEGIN_MAP_FOREACH(m, a, b, name)                                  \
    for (SCML_MAP(a, b)::iterator _iter_e = m.begin(); _iter_e != m.end();     \
         _iter_e++) {                                                          \
        b &name = _iter_e->second;
#define SCML_END_MAP_FOREACH }
#define SCML_BEGIN_MAP_FOREACH_CONST(m, a, b, name)                            \
    for (SCML_MAP(a, b)::const_iterator _iter_e = m.begin();                   \
         _iter_e != m.end(); _iter_e++) {                                      \
        b const &name = _iter_e->second;
                  ...
                  public:
                    ...
                    class Object_Container {
                      public:
                        ...
                        Object_Container() : object(NULL), object_ref(NULL) {}
                    ...
                    class Bone_Container {
                      public:
                        ...
                        Bone_Container() : bone(NULL), bone_ref(NULL) {}
                        Bone_Container(Bone *bone)
                            : bone(bone), bone_ref(NULL) {}
                        Bone_Container(Bone_Ref *bone_ref)
                            : bone(NULL), bone_ref(bone_ref) {}

                        bool hasBone() const { return (bone != NULL); }
                        bool hasBone_Ref() const { return (bone_ref != NULL); }
                    };
                  ...
                  public:
                    ...
                    class Object {
                      public:
                        ...
                        int animation;
                        float t;
                        // int z_index; // Does this exist?  Object_Ref has it,
                        // so probably not.
                        float volume;
                        float panning;
                        Meta_Data_Tweenable *meta_data;
...
/*! \brief A storage class for images in a renderer-specific format (to be
 * inherited).
 */
class FileSystem {
  public:
    ...
    /*! \brief Loads an image from a file and stores it so that the folderID and
     * fileID can be used to reference the image. \param folderID Integer folder
     * ID \param fileID Integer file ID \param filename Path of the image file
     * \return true on success, false on failure
     */
    virtual bool loadImageFile(int folderID, int fileID,
                               const SCML_STRING &filename) = 0;

    /*! \brief Cleans up all memory used by the FileSystem to store images,
     ...
     * \return A pair consisting of the width and height of the image.  Returns
     * (0,0) on error.
     */
    virtual SCML_PAIR(unsigned int, unsigned int)
        getImageDimensions(int folderID, int fileID) const = 0;
};
