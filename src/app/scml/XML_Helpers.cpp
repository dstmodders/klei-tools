...
// This is from Valentin Milea on Stack Overflow.
...
int toInt(const SCML_STRING &str) { return atoi(SCML_TO_CSTRING(str)); }

float toFloat(const SCML_STRING &str) {
    return (float)atof(SCML_TO_CSTRING(str));
}

SCML_STRING toString(bool b) { return (b ? "true" : "false"); }
...
char *stripTrailingDecimalZeros(char *buf) {
    ...
    // If there is a decimal part, remove all the trailing zeros and the decimal
    // point if we reach it.
    if (hasDecimal) {
        while (c != buf) {
            c--;
            if (*c == '0')
...
SCML_STRING xmlGetStringAttr(TiXmlElement *elem, const SCML_STRING &attribute,
                             const SCML_STRING &defaultValue) {
    const char *attr = elem->Attribute(SCML_TO_CSTRING(attribute));
    if (attr == NULL)
        return defaultValue;
    return attr;
