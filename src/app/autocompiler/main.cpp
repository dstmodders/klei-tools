...
struct compiler {
    compiler(Path const &compiler_path) : path(compiler_path) {
    ...
    void compile(char const *asset_path, char const *output_folder) {
        ...
        sprintf(command_line, "\"\"%s.exe\" \"%s\" \"%s\"\"", path.c_str(),
                asset_path, output_folder);
#else
        sprintf(command_line, "\"%s\" \"%s\" \"%s\"", path.c_str(), asset_path,
                output_folder);
#endif
            ...
            if (tinydir_readfile(&dir, &file) != -1) {
                ...
                if (file.is_dir) {
                    ...
                    if (name_length > extension_length &&
                        strcmp(file.name + name_length - extension_length,
                               extension) == 0) {
...
void compile_folder(compiler *c, Path const &asset_folder,
                    Path const &mod_folder) {
    // printf("compiling folder: %s For ext: %s\n", asset_folder.c_str(),
    // c->extension.c_str());
    file_list files;
    list_files(asset_folder, c->extension.c_str(), files);
    for (file_list::iterator file_iter = files.begin();
...
int main(int argument_count, char **arguments) {
    ...
    mods_folder =
        app_folder / ".." / ".." / "Don't Starve Together Beta" / "mods";
    list_folders(mods_folder, mod_folders);
    ...
    for (compiler_list::iterator iter = compilers.begin();
         iter != compilers.end(); ++iter) {
        ...
        for (file_list::iterator mod_folder_iter = mod_folders.begin();
             mod_folder_iter != mod_folders.end(); ++mod_folder_iter) {
            // printf("mod folder loop: %s\n", mod_folder_iter->c_str());
            for (file_list::iterator asset_folder_iter = asset_folders.begin();
                 asset_folder_iter != asset_folders.end();
                 ++asset_folder_iter) {
                // printf("asset loop: %s\n", asset_folder_iter->c_str());
                compile_folder(c, (*mod_folder_iter) / (*asset_folder_iter),
                               *mod_folder_iter);
            }
        }
