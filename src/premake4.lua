...
local hostIsUnix, hostIsWindows -- luacheck: ignore
do
...
local function rmSecondArgument(_, y)
    return rmDirTree(y)
end
...
local extract = (function()
    ...
    if hostIsUnix() then
        --[[
        -- Mac OS X and almost all Linux distributions have an unzip command.
        -- The only Linux distros which don't come with unzip are "power user"
        -- distros, in which either:
...
extract(
    ...
    catfile(props.outdir, "dont_starve", "mods")
)
