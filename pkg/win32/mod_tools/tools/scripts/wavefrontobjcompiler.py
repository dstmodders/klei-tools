...
def ParseFace(face, face_data):
    ...
    for vertex in face_data.split(" ")[1:]:
        ...
        # According to the waveform obj file format, you can have a vertex that
        # looks like 1//1 if the vertex has no texcoords, so we need to handle
        # that case by doing extra validation.

        # Also note that the obj file format uses 1 based indices, but python is
        # 0 based, hence the -1s below.
        pos_index = -1
        uv_index = -1
        normal_index = -1
        ...
        # If any of these are true then we really want to output different
    ...
    assert len(
        face.Vertices) == 3  # otherwise we have a non-triangular face and more
    # code needs to be implemented


def ParseMesh(mesh, material_name, face_data, _):
...
def ParseModel(model, filename):
    # The GenericGenerator is a helper function used to parse data from an obj
    # file It takes a list of strings, a prefix to search for an an obj type to
    # covert to, then whenever the prefix is encoutered it will generate the obj
    # and return it to you through an iterator interface.
    ...
    with open(filename, "rt") as f:
        ...
        # sped up by iterating just once. The change would make for uglier code
        # though.

        # Read all the raw data from the file (positions, uvs, normals).
        model.Positions = [Vector3([v.x, v.y, v.z]) for v in
                           GenericGenerator(lines, "v ",
                                            ModelCompiler.Position)]
        model.UVs = [v for v in
                     GenericGenerator(lines, "vt ", ModelCompiler.UV)]
        model.Normals = [v for v in
                         GenericGenerator(lines, "vn ", ModelCompiler.Normal)]

        # Now iterate through the file looking for the structures that use the
        # raw information built in the previous step.
        for line_idx in range(len(lines)):
            line = lines[line_idx].strip()
            if line == "" or line[0] == "#":
                continue

            # If we find a usemtl line, find all the faces following that line
            # and make a mesh out of them.
            if line.startswith("usemtl "):
                mesh_end_idx = line_idx + 1
                has_g = lines[mesh_end_idx].startswith("g ")
                while mesh_end_idx < len(lines) and not has_g:
                    mesh_end_idx += 1

                mesh = ModelCompiler.Mesh()
                ParseMesh(mesh, line.split(" ")[1],
                          lines[line_idx + 1: mesh_end_idx], model.Vertices)
                model.Meshes += [mesh]
                line_idx = mesh_end_idx
            elif line.startswith("mtllib "):
                materiallib = ModelCompiler.MaterialLib()
                ParseMaterialLib(materiallib,
                                 os.path.join(WorkingDir, line.split(" ")[1]))
...
def ParseMaterial(material, material_lines):
    ...
    for line in material_lines[1:]:
        ...
        #
        # Specifies that a color texture file or color procedural texture file
        # is linked to the diffuse reflectivity of the material. During
        # rendering, the map_Kd value is multiplied by the Kd value. "filename"
        # is the name of a color texture file (.mpc), a color procedural texture
        # file (.cxc), or an image file. The options for the map_Kd statement
        # are listed below. These options are described in detail in "Options
        # for texture map statements" on page 5-18.
        #
        # -blendu on | off
        # -blendv on | off
        # -cc on | off
        ...
        if line.startswith("map_Kd"):
            ...
            args = line.split(" ")[1:]
            material.Diffuse = Colour(float(args[0]), float(args[1]),
                                      float(args[2]), 1.0)
...
def main():
    ...
    if args.outfile is None:
        zf, _ = os.path.splitext(args.infile)
        args.outfile = zf + ".zip"
