...
def GetDim(ims, alignto, maxtexturesize=2048, scale_factor=1):
    area = reduce(lambda tally, next: tally + next.size[0] * next.size[
        1] * scale_factor * scale_factor, ims, 0)
    maxdim = NextMultipleOf(
        max([max(img.size[0], img.size[1]) * scale_factor for img in ims]),
        alignto)
    dim = min(maxtexturesize,
              max(math.pow(2, math.ceil(math.log(math.sqrt(area * 1.25), 2))),
                  math.pow(2, math.ceil(math.log(maxdim, 2)))))
...
def TryInsertImage(w, h, fblist, atlassize):
    ...
    while y + h < atlassize:
        ...
        if min_y:
            ...
            y += align
        x = 0
        # mylist = [fb for fb in mylist if BBoxIntersects(
        #     fb.bbox,
        #     BBox(0, y, atlassize, atlassize - y)
        # )]
...
def GenerateXMLTree(texture_filename, texture_size, bboxes, offset_amount=None):
    ...
    # pull in the UVs by a half pixel from the edge to avoid some sampling
    # issues, unless told otherwise
    offset_amount_x = offset_amount if offset_amount is not None else 0.5
    offset_amount_y = offset_amount if offset_amount is not None else 0.5

    border_uv_offset = (
        offset_amount_x / texture_size[0], offset_amount_y / texture_size[1])
    ...
    for filename, bbox in bboxes.items():
        ...
        u1 = Clamp(0.0, 1.0,
                   bbox.x / float(texture_size[0]) + border_uv_offset[0])
        v1 = Clamp(0.0, 1.0, 1.0 - (bbox.y + bbox.h) / float(texture_size[1]) +
                   border_uv_offset[1])

        u2 = Clamp(0.0, 1.0, (bbox.x + bbox.w) / float(texture_size[0]) -
                   border_uv_offset[0])
        v2 = Clamp(0.0, 1.0,
                   1.0 - bbox.y / float(texture_size[1]) - border_uv_offset[1])
...
def Atlas(ims,
    ...
    def LocalAtlas(im, source_idx):
        if im.size[0] > size[0] or im.size[1] > size[1]:
            sys.stderr.write(
                "Error: image " + im.name + " is larger than the atlas size!\n")
            sys.exit(2)
        ...
        for idx, fb in fullboxes.items():
            ...
            insertbbox = TryInsertImage(im.size[0], im.size[1], fblist,
                                        atlassize)
            if insertbbox:
                inserted = True
        ...
        if not inserted:
            ...
            newsize = GetDim(ims[source_idx:], blocksize, max_size,
                             scale_factor)
            fullboxes[numoutimages] = ((newsize, newsize), [
                FullBox(im, numoutimages, BBox(0, 0, im.size[0], im.size[1]),
                        im.name)])
    ...
    # now that we've figured out where everything goes, make the output images
    # and blit the source images to the appropriate locations
    ...
    for idx, fb in fullboxes.items():
        ...
        while w >= 1 or h >= 1:
            outim = OutImage("{0}-{1}".format(outname, idx),
                             Image.new("RGBA", (w, h)))
