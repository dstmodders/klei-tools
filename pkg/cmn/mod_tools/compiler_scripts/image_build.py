...
def ExportImage(filename):
    ...
    if util.IsAnyFileNewer(filename, dest_filename) or args.force:
        ...
        assert sz[0] % 4 == 0 and sz[1] % 4 == 0, \
            "Image " + filename + " has a dimension that is not a multiple of 4"
...
def process_file_to_atlas():
    for image in in_images:
        ...
        # if not util.IsAnyFileNewer(image, [
        #     dest_filename,
        #     xml_filename,
        # ]) or args.force:
        #     continue
        ...
        try:
            ...
            atlas_data = atlas.Atlas(images, image_base_name,
...
# This function is not used and so is probably inaccurate. If you ever want to
# process an entire directory to a single atlas this is a good starting spot
# though.
def process_dirs_to_atlas():
    # pylint: disable=undefined-variable
    for dirname in [d for d in os.listdir('.') if
                    os.path.isdir(d) and not d in excluded_dirs]:
