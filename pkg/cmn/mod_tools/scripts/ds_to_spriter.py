...
for symbol in build:
    ...
    for frame in symbol:
        ...
        image_path = os.path.split(sys.argv[1])[0] + '//' + symbol.get(
            'name') + '/' + image_name
        x = float(frame.get('x'))
        y = float(frame.get('y'))
        w = float(frame.get('w'))
        ...
        f = et.SubElement(folder, 'file')
        f.set('id', str(file_id))
        f.set('name', image_path[image_path.find('/') + 2:])
        f.set('width', str(int(w)))
        f.set('height', str(int(h)))
        f.set('pivot_x', pivot_x)
        f.set('pivot_y', pivot_y)
        files[str(file_id)] = f
        file_id = file_id + 1
        ...
        for ele in setup:
            if ele.get('name') in folders:
                ...
                if ele.get('frame') in files:
                    ...
                    timeline.set('name', ele.get('name') + '_' + ele.get(
                        'frame') + '_' + str(timeline_id))
