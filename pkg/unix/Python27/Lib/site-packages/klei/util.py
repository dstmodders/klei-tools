...
def IsFileNewer(src_path, dest_path):
    ...
    for root, _, files in os.walk(src_path):
        for filename in files:
            src_time = max(os.stat(os.path.join(root, filename)).st_mtime,
                           src_time)
    dest_time = None if not os.path.exists(dest_path) else os.stat(
        dest_path).st_mtime

    return dest_time is None or src_time > dest_time
