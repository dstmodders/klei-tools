...
def strhash(_str, hashcollection):
    _hash = 0
    for c in _str:
        v = ord(c.lower())
        _hash = (v + (_hash << 6) + (_hash << 16) - _hash) & 0xFFFFFFFFL
    hashcollection[_hash] = _str
    return _hash
...
def ExportAnim(endianstring, xmlstr, outzip, ignore_exceptions):
    ...
    outfile.write(
        struct.pack(endianstring + 'cccci', 'A', 'N', 'I', 'M', ANIMVERSION))
    outfile.write(struct.pack(endianstring + 'I',
                              len(doc.getElementsByTagName("element"))))
    outfile.write(
        struct.pack(endianstring + 'I', len(doc.getElementsByTagName("frame"))))
    outfile.write(
        struct.pack(endianstring + 'I', len(doc.getElementsByTagName("event"))))
    outfile.write(
        struct.pack(endianstring + 'I', len(doc.getElementsByTagName("anim"))))

    def LocalExport(anim_node):
        name = anim_node.attributes["name"].value.encode('ascii')

        dirs = (re.search(r"(.*)_up\Z", name),
                re.search(r"(.*)_down\Z", name),
                re.search(r"(.*)_side\Z", name),
                re.search(r"(.*)_left\Z", name),
                re.search(r"(.*)_right\Z", name),
                re.search(r"(.*)_upside\Z", name),
                re.search(r"(.*)_downside\Z", name),
                re.search(r"(.*)_upleft\Z", name),
                re.search(r"(.*)_upright\Z", name),
                re.search(r"(.*)_downleft\Z", name),
                re.search(r"(.*)_downright\Z", name),
                re.search(r"(.*)_45s\Z", name),
                re.search(r"(.*)_90s\Z", name))

        facingbyte = FACING_RIGHT | FACING_LEFT | \
        ...
        outfile.write(
            struct.pack(endianstring + 'I', strhash(root, hashcollection)))
        outfile.write(
            struct.pack(endianstring + 'fI', float(frame_rate), num_frames))
        for frame_node in anim_node.getElementsByTagName("frame"):
            ...
            for event_node in frame_node.getElementsByTagName("event"):
                outfile.write(struct.pack(endianstring + 'I', strhash(
            ...
            for element_node in elements:
                outfile.write(struct.pack(endianstring + 'I', strhash(
                    element_node.attributes["name"].value.encode('ascii'),
                    hashcollection)))
                outfile.write(struct.pack(endianstring + 'I', int(
                    element_node.attributes["frame"].value)))
                layername = element_node.attributes["layername"].value.encode(
                    'ascii').split('/')[-1]
                outfile.write(struct.pack(endianstring + 'I',
                                          strhash(layername, hashcollection)))

                z = (eidx / float(num_elements)) * float(
                    EXPORT_DEPTH) - EXPORT_DEPTH * .5
                outfile.write(struct.pack(
                    endianstring + 'fffffff',
...
def AtlasImages(ims,
    ...
    if len(ims) > 0:
        ...
        try:
            # convert all of the result images to the game format, and save them
            # to the zip file

            for _, atlasdata in atlases.items():
                tex_filename = atlasdata.mips[0].name + ".tex"
                dest_filename = os.path.join(temp_dir, tex_filename)

                mip_filenames = [os.path.join(temp_dir, atlasdata.mips[
                    0].name + "_" + str(mip_idx) + ".png") for mip_idx in
                                 range(len(atlasdata.mips))]

                valid_filenames = []
                textureformat = textureformat if antialias \
                    else hardalphatextureformat
                ...
                for mip_idx in range(len(atlasdata.mips)):
                    ...
                    filter_func = Image.ANTIALIAS if antialias \
                        else Image.NEAREST
                    mip_img = mip[1]
...
def AddVertsToVB(image_name,
    ...
    for atlas_idx in range(len(atlases)):
        _atlas = atlases[atlas_idx]
        bboxes = _atlas.bboxes
        src_images = _atlas.src_images

        sampler = atlas_idx + sampleroffset

        imsizex = float(_atlas.mips[0].im.size[0])
        imsizey = float(_atlas.mips[0].im.size[1])
        ...
        for img in src_images:
            ...
            #     cropped = img.crop((region.x, region.y, region.x + region.w,
            #                         region.y + region.h))
            #     new_img.paste(cropped, (region.x,
            #                             region.y,
            #                             region.x + region.w,
            #                             region.y + region.h))
            ...
            for region in src_regions:
                ...
                umin = max(0.0, min(
                    1.0,
                    (dest_bbox.x + region.x) / imsizex
                ))

                umax = max(0.0, min(
                    1.0,
                    (dest_bbox.x + region.x + region.w) / imsizex
                ))

                vmin = max(0.0, min(
                    1.0,
                    1 - (dest_bbox.y + region.y) / imsizey
                ))

                vmax = max(0.0, min(
                    1.0,
                    1 - (dest_bbox.y + region.y + region.h) / imsizey
                ))
...
def ExportBuild(endianstring,
    ...
    imnames = {frame.attributes["image"].value.encode('ascii') for frame in
    ...
    atlases = AtlasImages(images, "atlas", outzip, 2048, antialias, platform,
                          textureformat, hardalphatextureformat, force,
                          ignore_exceptions)
    ...
    outfile.write(
        struct.pack(endianstring + 'cccci', 'B', 'I', 'L', 'D', BUILDVERSION))

    outfile.write(struct.pack(endianstring + 'I',
                              len(doc.getElementsByTagName("Symbol"))))
    outfile.write(
        struct.pack(endianstring + 'I', len(doc.getElementsByTagName("Frame"))))

    buildname = doc.getElementsByTagName("Build")[0].attributes[
        "name"].value.encode('ascii')
    buildname = os.path.splitext(buildname)[0]
    outfile.write(struct.pack(endianstring + 'i' + str(len(buildname)) + 's',
                              len(buildname), buildname))
    ...
    symbol_nodes = sorted(doc.getElementsByTagName("Symbol"),
                          key=lambda x: strhash(
                              x.attributes["name"].value.encode('ascii'),
                              hashcollection))

    for symbol_node in symbol_nodes:  # doc.getElementsByTagName("Symbol"):
        symbolname = symbol_node.attributes["name"].value.encode('ascii')
        outfile.write(struct.pack(endianstring + 'I',
                                  strhash(symbolname, hashcollection)))
        outfile.write(struct.pack(endianstring + 'I',
                                  len(symbol_node.getElementsByTagName(
                                      "Frame"))))
        ...
        for frame_node in symbol_node.getElementsByTagName("Frame"):
            ...
            alphacount = AddVertsToVB(imagename, "alpha", atlases, xoff, yoff,
                                      z, alphaverts)

            # opaqueidx = len(opaqueverts)
            alphacount += AddVertsToVB(imagename, "opaque", atlases, xoff, yoff,
                                       z, alphaverts)
...
def main(_args):
    ...
    try:
        ...
        if _args.scale:
            SCALE_FACTOR = _args.scale

        if _args.verbosity:
            VERBOSITY = _args.verbosity

        path, base_name = os.path.split(_args.infile)
        base_name, ext = os.path.splitext(base_name)
        path = os.path.abspath(path)

        if ext != ".zip":
            sys.stderr.write("I only accept zip files, and you tried to make "
                             "me process " + _args.infile + "\n")
            sys.exit()

        data_path = os.path.abspath(os.path.join(path, "..", _args.outputdir))
        zip_file = zipfile.ZipFile(_args.infile, "r")
        ...
        if "animation.xml" in zip_file.namelist():
            ...
            ExportAnim(endianstring, animxml, outzip, _args.ignoreexceptions)

        if "build.xml" in zip_file.namelist():
            buildxml = zip_file.read("build.xml")
            ExportBuild(endianstring, zip_file, buildxml, outzip,
                        not _args.skipantialias, _args.platform,
                        _args.textureformat, _args.hardalphatextureformat,
                        _args.force, _args.ignoreexceptions)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Export an animation from '
                                                 'xml/png intermediates to '
                                                 'game data.')
    parser.add_argument('infile', action="store")
    parser.add_argument('--bigendian', action="store_true", default=False)
    parser.add_argument('--scale', action='store', default=1.0, type=float,
                        required=False)
