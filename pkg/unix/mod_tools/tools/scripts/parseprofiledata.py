...
ProfileEntry = collections.namedtuple("ProfileEntry",
                                      "totalCalls, timeTaken, minTime, maxTime")
format_str = "{0:30s}  {1:<16} {2: =12.3f} {3: =12.3f} {4: =12.3f}"


def PrintFrame(frame):
    print("{0:30s}  {1:<16} {2:>12} {3:>12} {4:>12}".format("Name", "# Calls",
                                                            "Total (ms)",
                                                            "Min (ms)",
                                                            "Max (ms)"))
    ...
    for idx, entry in frame:
        indent = ""
        for _ in range(ProfileRegions[idx].indentation):
            indent += " "
        print(format_str.format(indent + ProfileRegions[idx].name,
                                entry.totalCalls, entry.timeTaken * 1000,
                                entry.minTime * 1000, entry.maxTime * 1000))
...
def main():
    ...
    with file(args.infile, "rb") as f:
        ...
        while data_offset < data_len:
            ...
            try:
                for i in range(num_entries):
                    totalCalls, timeTaken, minTime, maxTime = entry_struct \
                        .unpack_from(data, data_offset)
                    data_offset += entry_struct.size

                    entry = ProfileEntry(totalCalls, timeTaken, minTime,
                                         maxTime)

                    # print "\t" + ", ".join((str(entry.totalCalls),
                    #                         str(entry.timeTaken),
                    #                         str(entry.minTime),
                    #                         str(entry.maxTime)))
