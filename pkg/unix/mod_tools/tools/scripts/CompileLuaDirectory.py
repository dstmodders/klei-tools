...
# * This function compiles all files with a .lua file extension in the current
# * directory, via luac. This function will also attempt to backup the scripts,
# * if requested.
# *
...
def compileLuaDirectory(resourceDirectory=os.getcwd()):
    ...
    userInput = raw_input(
        "* Would you like to back up %s? All current backups will be lost! ["
        "y/n]\n" % (
            resourceDirectory))
    if userInput == "y":
    ...
    for currentDirectory, _, filePaths in os.walk(resourceDirectory):
...
# * This function backups all files in the current directory to the src-backup
# * folder.
# *
# * @return True if successful, False otherwise.
def backupDirectory(directoryToBackup):
    ...
    try:
        ...
        print "ERROR: Directory backup failed! Please ensure the backup " \
...
def main():
    print(
        "== Lua Directory Compiler by John_Michael <JohnMichaelFFS@gmail.com> "
        "==\n")
    compileLuaDirectory()
    os.system('pause')
