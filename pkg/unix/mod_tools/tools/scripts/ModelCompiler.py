...
def WriteString(stream, string):
    packed_data = struct.pack(EndianString + "I" + str(len(string)) + "s",
                              len(string), string)
...
class Face(object):
    """
    A face is composed of a set of vertices which are a combination of
    pos/uv/normal indices into the global pos/uv/normal tables.
    """
    ...
    def __str__(self):
        ...
        s = "Face: NumVertices( {0} )".format(
            len(self.Vertices)) + "\n\t\t".join(
            [""] + [str(v) for v in self.Vertices])
        return s
...
class Mesh(object):
    """
    Meshes are simply index buffers that reference the raw vertex data in the
    parent model.
    """
    ...
    def __str__(self):
        s = "Mesh - Material( {0} ), Faces( {1} )".format(self.MaterialName,
                                                          len(self.Faces))
    ...
    def Compile(self, stream, model_vertices, materials):
        ...
        for face in self.Faces:
            ...
            for vtx in face.Vertices:
                ...
                # TODO: Figure out if this should be a binary search. If it's
                #  linear it's probably quite slow
        ...
        #  for there to be duplicate vertices though it is unlikely. To be
        #  completely "correct" you would do a post pass to check for dupes
        #  based on values
...
class Model(object):
    ...
    def Compile(self, filename):
        ...
        for pos in self.Positions:
            self.MinPosition = self.MinPosition.Min(
                Vector3([pos.x, pos.y, pos.z]))
            self.MaxPosition = self.MaxPosition.Max(
                Vector3([pos.x, pos.y, pos.z]))

        with zipfile.ZipFile(filename, "w", zipfile.ZIP_DEFLATED) as zf:
            # TODO: Write out the vertex description so it's not hard coded
            #  in the game

            # Be very careful re-ordering anything below! Many of these
            # compile calls have side effects.
            ...
            for mesh in self.Meshes:
                mesh.Compile(meshes, self.Vertices,
                             self.MaterialLib.Materials.keys())
...
class Material(object):
    """
    To properly populate a material you want to fill out the TextureFilenames
    map with a tuple of ( src_filename, dest_filename). Then, when compile is
    called the texture converter will be called and everything will be properly
    baked/zipped.

    filename is the name of the file to be used in the zip (i.e. does not
    contain a path) src_filename is the relative or absolute path to the
    PNG/source texture dest_filename should be the path & filename to a
    temporary file to be used for the texture conversion process. This file WILL
    be removed.
    """
    ...
    def Compile(self, stream):
        ...
        for _, filenames in self.TextureFilenames.items():
            src_filename, dest_filename = filenames

            _ = Image.open(src_filename)
            pixel_format = "bc3"
            # if img.mode == "RGB":
            #     pixel_format = "bc1"

            # TODO: Output to an intermediate directory and do timestamp
            #  checking
            print "Converting " + src_filename
            try:
                textureconverter.MipAndConvert(src_filename, dest_filename,
                                               pixel_format)
