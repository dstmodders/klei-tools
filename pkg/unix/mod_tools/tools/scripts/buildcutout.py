...
from pipelinetools import GetBaseDirectory, TrimImage, VerifyDirectory


def MakeObjQuad(origsize, trimsize, scale, depth, plane, outfilename):
    p0 = [(trimsize[0] - origsize[0] / 2) * scale,
          (-trimsize[3] + origsize[1] / 2) * scale, depth]
    p1 = [(trimsize[0] - origsize[0] / 2) * scale,
          (-trimsize[1] + origsize[1] / 2) * scale, depth]
    p2 = [(trimsize[2] - origsize[0] / 2) * scale,
          (-trimsize[1] + origsize[1] / 2) * scale, depth]
    p3 = [(trimsize[2] - origsize[0] / 2) * scale,
          (-trimsize[3] + origsize[1] / 2) * scale, depth]
...
def ProcessFile(_file, options):
    ...
    print "Exporting " + _file

    basedir = GetBaseDirectory(_file, "contentsrc")
    _ = VerifyDirectory(basedir, "tools")
    outdir = VerifyDirectory(basedir, "intermediates/models")
    tempdir = tempfile.mkdtemp()

    # trim the file, save the size, save the image
    origsize, trimsize, trimmed = TrimImage(_file)
    ...
    MakeObjQuad(origsize, trimsize, options.scale, options.depth, options.plane,
                tempobjname)
...
def main():
    ...
    parser = optparse.OptionParser(
        description='Process image files into intermediate obj+cropped texture'
                    'pairs')

    parser.add_option('-s',
                      '--scale',
                      help="scale applied to the model to bring it from pixel "
                           "space",
                      default=1,
                      type=float)
    parser.add_option('-d',
