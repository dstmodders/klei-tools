...
class QuadTreeNode(object):
    ...
    def __init__(self, im, bbox=None, depth=0, blocksize=32):
        ...
        if bbox.w > self.blocksize and bbox.h > self.blocksize:
            ...
            childboxes = [BBox(bbox.x, bbox.y, bbox.w / 2, bbox.h / 2),
                          BBox(bbox.x + bbox.w / 2, bbox.y, bbox.w - bbox.w / 2,
                               bbox.h / 2),
                          BBox(bbox.x, bbox.y + bbox.h / 2, bbox.w / 2,
                               bbox.h - bbox.h / 2),
                          BBox(bbox.x + bbox.w / 2, bbox.y + bbox.h / 2,
                               bbox.w - bbox.w / 2, bbox.h - bbox.h / 2)]
            ...
            same_type = childtypes[0] == childtypes[1] == childtypes[2] == \
                        childtypes[3]
            last_div = bbox.w / 2 < self.blocksize or (
                bbox.h / 2 < self.blocksize
            )
            ...
            if self.children and len(self.children) == 4 and self.children[
                0].type == self.children[1].type == self.children[2].type == \
                self.children[3].type == BlockType.ALPHA:
                self.children = None
    ...
    def __repr__(self):
        if self.children is None:
            return "\t" * self.depth + BlockName(self.type) + " " + str(
                self.bbox)
...
def optlist(orig):
    ...
    for o in orig:
        added = False
        for n in newlist:
            is_same_x = n.x + n.w == o.x or o.x + o.w == n.x
            is_same_y = n.y + n.h == o.y or o.y + o.h == n.y

            if n.w == o.w and n.x == o.x and is_same_y:
                newlist.remove(n)
                newlist.append(BBox(n.x, min(o.y, n.y), n.w, o.h + n.h))
                added = True
                break
            if n.h == o.h and n.y == o.y and is_same_x:
                newlist.remove(n)
...
def GetImageRegions(img, blocksize=16):
    ...
    opaque = doopt(
        sorted(rootNode.GetBBox(lambda x: x.type == BlockType.OPAQUE),
               key=lambda x: x.w * x.h,
               reverse=True))
    alpha = doopt(sorted(rootNode.GetBBox(lambda x: x.type == BlockType.ALPHA),
                         key=lambda x: x.w * x.h,
                         reverse=True))
