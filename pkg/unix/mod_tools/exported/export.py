...
class Prefabs(object):
    ...
    def ParsePrefabs(self, filename):
        ...
        for prefab in prefabs:
            ...
            assets = [Asset(asset.attrib['file'], asset.attrib['type'].upper())
                      for asset in prefab.findall("Assets/Asset")]
...
def ConvertAnim(params):
    ...
    anim_name, _ = os.path.splitext(anim_filename)
    ...
    cmd_args = [
        sys.executable,
        os.path.abspath(os.path.join(SCRIPT_DIR, "..", "tools", "scripts",
                                     "buildanimation.py")),
        "--scale=" + str(scale)
    ]
    ...
    is_new = util.IsFileNewer(src_file, dest_file)
    if args.force or not os.path.exists(dest_file) or is_new:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Animation batch exporter script")
    ...
    parser.add_argument('anims',
                        type=str,
                        nargs='?',
                        help="List of anim filenames to be exported")
    ...
    prefabs_file = os.path.abspath(
        os.path.normpath(os.path.join(CWD, args.prefabsdir, "prefabs.xml")))
    prefabs = Prefabs(prefabs_file)
    anims = []
    if args.anims:
        anims = [args.anims]
    else:
        anims = [os.path.basename(anim.File) for anim in
                 prefabs.GetAssetType("anim")]
